...
class HybridTrainer:
    ...
    def fit_spatial_engine(self) -> None:
        ...
        # Save fitted models
        save_dir = self.config.get('hybrid', {}).get('spatial_reasoning', {}).get(
    ...
    def train_fusion(self) -> Dict[str, Any]:
        ...
        scheduler_3a = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer_3a, T_max=frozen_epochs, eta_min=1e-6
        )
        ...
        for epoch in range(1, frozen_epochs + 1):
            ...
            # Log
            entry = {'epoch': epoch, 'stage': '3a', 'train': train_losses,
        ...
        scheduler_3b = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer_3b, T_max=finetune_epochs, eta_min=1e-7
        )
        ...
        for epoch in range(frozen_epochs + 1, total_epochs + 1):
            ...
            entry = {'epoch': epoch, 'stage': '3b', 'train': train_losses,
    ...
    def _train_epoch(self, optimizer: torch.optim.Optimizer,
        ...
        running_losses = {
            'total': 0.0, 'yolact': 0.0, 'spatial_consistency': 0.0,
            'cls': 0.0, 'box': 0.0, 'mask': 0.0,
        }
        num_batches = 0
        ...
        for batch_idx, (images, targets) in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % 50 == 0:
                ...
                logger.info(
                    ...
                    epoch, total_epochs, batch_idx + 1,
                    len(self.train_loader), avg_loss, gate_val
                )
        ...
        logger.info(
            "Epoch [%d/%d] Train Loss: %.4f (yolact: %.4f, spatial: %.4f) "
            "Gate: %.4f Time: %.0fs",
            epoch, total_epochs, avg_losses['total'], avg_losses['yolact'],
            avg_losses['spatial_consistency'], avg_losses['gate_value'], epoch_time
        )
    ...
    def _validate_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        logger.info(
            ...
            avg_losses['total'], avg_losses['yolact'],
            avg_losses['spatial_consistency'], val_time
        )

        return avg_losses

    def _save_checkpoint(self, epoch: int, optimizer: torch.optim.Optimizer,
                         is_best: bool = False, filename: str = None) -> None:
