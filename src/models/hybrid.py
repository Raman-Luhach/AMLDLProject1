...
class HybridDetector(nn.Module):
    ...
    def __init__(
        ...
        self.spatial_engine = SpatialReasoningEngine(
            self.hybrid_config['spatial_reasoning']
        )
    ...
    def _extract_fpn_features(self, images: torch.Tensor) -> Tuple[
    ...
    def _get_spatial_features_from_boxes(
        ...
        """
        spatial_features = self.spatial_engine.compute_spatial_features(
            boxes, image_size=self._input_size
        )
    ...
    def _extract_visual_features(
        self, fpn_feature: torch.Tensor, boxes: torch.Tensor
    ) -> torch.Tensor:
        ...
        """
        if len(boxes) == 0:
            return torch.zeros(0, self.hybrid_config['recalibrator']['visual_dim'],
                             device=fpn_feature.device)
    ...
    def _training_forward(
        ...
        density_tensor = torch.tensor(
            np.stack(density_maps), dtype=torch.float32, device=device
        ).unsqueeze(1)

        # Convert spatial features to tensor (B, 8)
        spatial_tensor = torch.tensor(
            np.stack(spatial_features_batch), dtype=torch.float32, device=device
        )
    ...
    def _inference_forward(
        ...
        class_preds_init, box_preds_init, mask_coeffs_init = \
            self.yolact.prediction_head(fpn_features)
        ...
        for i in range(batch_size):
            ...
            density_tensor = torch.tensor(
                density, dtype=torch.float32, device=device
            ).unsqueeze(0).unsqueeze(0)

            attended_p3 = self.spatial_attention(
                fpn_features[0][i:i+1], density_tensor
            )
            ...
            refined_dets = self.yolact.detect(
                class_ref, box_ref, mask_ref, prototypes_refined, anchors
            )
            ...
            visual_feats = self._extract_visual_features(
                attended_p3,
                refined_boxes / input_size  # normalize to [0, 1]
            )

            # Expand spatial features to match number of detections
            spatial_tensor = torch.tensor(
                spatial_feats, dtype=torch.float32, device=device
            ).unsqueeze(0).expand(len(refined_boxes), -1)

            # Recalibrate
            recalibrated_scores = self.recalibrator(
                refined_scores, spatial_tensor, visual_feats
            ).squeeze(1)
