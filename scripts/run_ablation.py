...
def generate_ablation_charts(results, output_dir):
    ...
    for name in ['full_hybrid', 'dl_only', 'no_recalibrator', 'no_spatial_attention',
                  'no_row_model', 'no_density_field', 'hard_nms', 'no_cbam']:
        if name in results:
    ...
    if 'full_hybrid' in results and 'density_analysis' in results['full_hybrid']:
        ...
        for ax_idx, metric in enumerate(['mAP_50', 'recall', 'F1']):
            ...
            axes[ax_idx].bar(buckets, values, color=['#3498db', '#f39c12', '#e74c3c'],
...
def main():
    ...
    # Load spatial engine
    spatial_dir = config.get('hybrid', {}).get('spatial_reasoning', {}).get(
