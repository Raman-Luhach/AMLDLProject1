...
def draw_tensor_label(ax, x, y, text, fontsize=6):
    ...
    ax.text(x, y, text, ha='center', va='center', fontsize=fontsize,
            color='#7F8C8D', fontstyle='italic', zorder=4,
            bbox=dict(boxstyle='round,pad=0.15', facecolor='white',
                     edgecolor='#BDC3C7', linewidth=0.5, alpha=0.9))
...
def generate_phase3_diagram():
    ...
    # Title
    ax.text(8, 10.2, 'Phase 3: Hybrid Neuro-Symbolic Architecture',
            ha='center', va='center', fontsize=16, fontweight='bold',
            color='#2C3E50')
    ax.text(8, 9.8, 'YOLACT + Spatial Reasoning Engine + Differentiable Feedback',
    ...
    ax.text(2.0, 5.7, 'Spatial Reasoning Engine (ML)', ha='center', fontsize=9,
    ...
    draw_box(ax, 5.0, 3.5, 3.0, 1.2, 'Spatial Prior Attention\n(learnable gate)\nFEEDBACK LOOP', FUSION_COLOR, fontsize=8, text_color='white')
    ...
    ax.text(5.8, 6.2, 'FEEDBACK\n(modulates P3)', fontsize=7, color=FEEDBACK_COLOR,
            fontweight='bold', rotation=70)

    # Confidence Recalibrator
    draw_box(ax, 9.0, 3.5, 3.0, 1.5, 'Confidence\nRecalibrator\nMLP: [73] → 128 → 64 → 1', FUSION_COLOR, fontsize=8, text_color='white')
    ...
    for i, (color, label) in enumerate(legend_items):
        x = 0.5 + i * 3.5
        patch = FancyBboxPatch((x, legend_y), 0.6, 0.4, boxstyle="round,pad=0.05",
    ...
    draw_arrow(ax, 0.5, legend_y - 0.3, 1.3, legend_y - 0.3,
               color=FEEDBACK_COLOR, style='->', lw=2.0, dashed=True)
    ax.text(1.5, legend_y - 0.3, 'Feedback Loop (ML → DL)', fontsize=8,
            va='center', color=FEEDBACK_COLOR)

    # Total params
    ax.text(8, 0.5, 'Total: ~10.0M (YOLACT) + ~25K (Fusion) = ~10.03M params',
            ha='center', fontsize=10, color='#2C3E50', fontstyle='italic')

    # Save
    output_dir = os.path.join(project_root, 'results', 'figures')
    os.makedirs(output_dir, exist_ok=True)

    plt.savefig(os.path.join(output_dir, 'architecture_phase3.png'),
                dpi=300, bbox_inches='tight', facecolor=BG_COLOR)
    plt.savefig(os.path.join(output_dir, 'architecture_phase3.pdf'),
...
def generate_phase1_diagram():
    ...
    ax.text(7, 4.7, 'Phase 1: Classical ML Baseline (HOG + SVM)',
    ...
    ax.text(7, 0.5, 'Result: mAP@0.5 = 3.09% | Precision = 86.4% | Recall = 2.1%',
            ha='center', fontsize=10, color='#E74C3C', fontstyle='italic')

    output_dir = os.path.join(project_root, 'results', 'figures')
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(os.path.join(output_dir, 'architecture_phase1.png'),
                dpi=300, bbox_inches='tight', facecolor=BG_COLOR)
    plt.savefig(os.path.join(output_dir, 'architecture_phase1.pdf'),
                bbox_inches='tight', facecolor=BG_COLOR)
    plt.close()
...
def generate_phase2_diagram():
    ...
    ax.text(8, 8.2, 'Phase 2: YOLACT + MobileNetV3 + FPN + CBAM + Soft-NMS',
            ha='center', fontsize=14, fontweight='bold', color='#2C3E50')
    ...
    ax.text(8, 0.5, 'Total: ~10.0M params | ONNX INT8: 9.9MB | 8.7 FPS (CPU)',
            ha='center', fontsize=10, color='#2C3E50', fontstyle='italic')

    output_dir = os.path.join(project_root, 'results', 'figures')
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(os.path.join(output_dir, 'architecture_phase2.png'),
                dpi=300, bbox_inches='tight', facecolor=BG_COLOR)
    plt.savefig(os.path.join(output_dir, 'architecture_phase2.pdf'),
                bbox_inches='tight', facecolor=BG_COLOR)
    plt.close()
