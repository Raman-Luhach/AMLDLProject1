...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--config', type=str, default='configs/hybrid.yaml',
    ...
    parser.add_argument(
        '--yolact-checkpoint', type=str, default=None,
    ...
    parser.add_argument(
        '--spatial-models', type=str, default=None,
...
def main() -> None:
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        logger.info("Resumed from hybrid checkpoint: %s (epoch %d)",
                    args.resume, checkpoint.get('epoch', 0))
    ...
    logger.info("  Total: %s (trainable: %s)",
                format_params(params['total']), format_params(params['trainable']))
    ...
    if args.stage in ('train-hybrid', 'all'):
        ...
        if args.stage == 'train-hybrid':
            spatial_dir = args.spatial_models or config.get('hybrid', {}).get(
                'spatial_reasoning', {}).get('save_dir', 'results/hybrid/spatial_models')
